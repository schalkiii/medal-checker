...
| 调试能力 | 无 | 一键导出原始 HTML 调试包 + 本地分析脚本 |
| UI 设计 | 基础双栏 900px | 卡片式布局 1100px，渐变色按钮，勋章彩色标签 |
| 开发工具链 | 无 | ESLint + 语法检查 + 文件监控 + 52 项自动化测试 |
| 代码量 | ~600 行 | ~1200 行（含 600+ 行测试） |
...
1. 点击 **「检测站点 Cookie」** 按钮，自动识别已登录的 PT 站点
2. 检测结果会自动填充到站点列表并保存
3. 点击 **「开始扫描」**，等待扫描完成
...
- 完整测试套件（52 项测试）和 CI 检查流水线（24 项检查）
