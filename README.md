...
- 修复 `extractMedalsFromCards()` 泛化实际未生效的 bug（v1.4 文档化但代码未更新）
- 卡片标签检测从硬编码空格改为 `/medal-card\b/` 正则
- 购买按钮匹配泛化：`\bbuy\b` 匹配 `action-btn btn-primary buy` 等任意含 buy 的 class
- 名称提取 3 层回退：`medal-name` / `medal-title` / `medal-card__name` → img alt → 首 h1-h4
- 字段提取双模式：`<strong>标签:</strong>值`（旧）+ `<span>标签:</span><strong>值</strong>`（hxpt 新）
- 新增 hxpt.org（好学）卡片布局支持
- DEFAULT_SITES 更新：`agsvpt.com` → `agsvpt.cn`，`playletpt.xyz` → `playlet.cc`
- 勋章跳转链接精度修复：URL 已有 query string 时用 `&` 代替 `?`
- 新增 7 个 hxpt 卡片布局单元测试（总计 69 个）
