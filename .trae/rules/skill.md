...
## zmpt.cc 新版 CSS Modules 布局适配指南

### 问题描述

zmpt.cc 之前一直无法检测出勋章。原因是该站点使用 Vue/Vite 构建的 SPA，且渲染后的 DOM 使用 CSS Modules 布局（类名带 hash 后缀），既不是标准 medal-card 布局，也不是表格布局，导致 `extractMedalsFromCards()` 无法识别。

### 布局特征

```html
<div class="index-module__card-wrap___C0zIS">
  <div class="index-module__card___90vxo">
    <div class="index-module__img-box___nrscc">
      <img alt="" src="..." />
    </div>
    <div class="index-module__title___ecDG8">
      <div>勋章名称</div>
      <div class="index-module__title-description___kAu2l">描述</div>
    </div>
    <div class="index-module__time-limit___cv1Bw">
      <div>2025-06-04 00:00:00 - 2030-12-31 00:00:00</div>
    </div>
    <div class="index-module__desc___4OOeV">
      <div class="index-module__item___6A7a7">
        <div class="index-module__label___dRuRs">电力加成</div>
        <div class="index-module__content___Yr-LT">35%</div>
      </div>
      <div class="index-module__item___6A7a7">
        <div class="index-module__label___dRuRs">价格</div>
        <div class="index-module__content___Yr-LT">5000000</div>
      </div>
      <!-- ...有效期/库存/优先级等 -->
    </div>
    <div class="index-module__control___tcPgO">
      <button class="">购买</button>
      <button>赠送</button>
    </div>
  </div>
</div>
...
1. **结构特征双重检测**：必须同时包含 `index-module__card` 和 `index-module__label`，避免误匹配其他布局
2. **CSS Modules 类名带 hash 后缀**：如 `___90vxo`、`___dRuRs`，不能硬编码完整类名，必须用 `[^"]*` 通配
3. **容器识别**：`<div class="index-module__card-wrap` 是卡片最外层
4. **深度计数提取**：使用 `<div>` 深度计数而非正则，确保正确匹配嵌套 div 边界（与 si-qi/hhanclub 一致）
5. **按钮特征**：`<button class="">购买</button>`，class 为空字符串，文本在标签内（不是 value 属性）
6. **按钮值过滤**：仅提取文本为 `购买` 或 `購買` 且非 disabled 的按钮，过滤 `佩戴`/`已过可购买时间`/`取消佩戴`/`赠送` 等
7. **名称提取**：从 `index-module__title` 内的第一个 `<div>` 提取，回退到 `<img alt>`
8. **字段提取**：从 `index-module__label`/`index-module__content` 键值对按 label 关键词匹配（价格/價格/有效期/加成/库存/庫存）
9. **content 清理**：去除嵌套标签后，去除所有空白字符（`replace(/\s+/g, "")`），避免 " 无限" 等带空格的值

### 适配决策：针对性适配

采用独立提取器 `extractMedalsFromZmptNew()`，而非扩展 `extractMedalsFromCards()`，理由：

- CSS Modules 结构（`index-module__card-wrap`/`__label`/`__content`）与现有所有布局差异显著
- 与 pterclub/hhanclub/si-qi 等站点特异提取器的现有模式一致
- 零回归风险：通过结构特征双重检测，不会误匹配其他布局
...
1. 运行 `node scripts/analyze-html.js PT_Debug_*.json` 确认 zmpt 站点输出 "zmpt 新版 CSS Modules 布局"
2. 运行 `node tests/unit/background.test.js` 确认所有测试通过
3. 运行 `npm test` 确认全部通过

### 历史参考

- 2026-06-22: 适配 zmpt.cc 新版 CSS Modules 布局（v1.9）
  - 修改文件：`background.js`（新增 `extractMedalsFromZmptNew()` 函数 + SPA 流程集成 + 导出）
  - 同步更新：`scripts/analyze-html.js`（zmpt CSS Modules 布局检测分支）
  - 新增测试：12 个 zmpt 单元测试（97→109，覆盖可购买/混合场景/繁体/disabled/过期/佩戴/空HTML/误匹配防护/真实fixture）
  - 新增 fixture：`tests/fixtures/zmpt-rendered.html`（真实渲染后 HTML）
  - 适配决策：针对性适配（独立提取器），零回归风险
  - 修复点：之前 zmpt 一直无法检测勋章，原因是 SPA 渲染后 DOM 使用 CSS Modules 布局，旧提取器无法识别
