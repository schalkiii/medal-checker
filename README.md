...
| 维度 | 原版 | 现版 |
|------|------|------|
| 勋章识别 | 简单正则计数 `value="购买"` | 智能列式解析引擎，自动适配 9/10 列表格 + buycenter + 卡片布局 + pterclub 猫站 |
...
测试包括：
- 单元测试（54 项）：background.js 核心函数（勋章提取、卡片布局、列布局检测等）
...
- `extractMedalsFromHtml()` - 标准 NexusPHP 勋章解析，自动检测 9列/10列表格布局，提取名称/价格/有效期/加成/库存/可购买时间；同时支持卡片式勋章布局（`<div class="medal-card">`）
...
- 卡片布局按钮正则泛化：`class="btn buy"` → `\bclass="[^"]*\bbtn\b[^"]*\bbuy[^"]*"`，分别匹配 `btn` 和 `buy` 为独立单词
- 卡片边界检测修复：区分 `medal-card` 和 `medal-cards` 包装容器，避免整页卡片被当作一张
- 名称提取 4 层回退：div.medal-name → div.medal-card__name → h[2-4].medal-name → img alt
- 字段提取 6 层回退：meta-row(ptlgs) → meta-item(luckpt) → price-value → detail-item(longpt) → medal-addition(luckpt加成) → stat-item(cspt) → strong标签 → data属性
- 适配 cspt.top(9)、pt.luckpt.de(51)、ptlgs.org(1)、longpt.org(1)、playlet.cc(2) 共 5 个新站点
- 勋章总数 45 → 98（+53 新增），覆盖 17 个站点
- 测试保持 62/62 全通过，25 项检查全通过
...
- 名称提取 3 层回退：medal-name/medal-title/medal-card__name → img alt → 首 h1-h4
